...
trait ProvidesRevisionActionSchema
{
    protected function revisionActionSchema(JsonSchema $schema): ObjectType
    {
        return $schema->object([
            ...
            'value' => $schema->union(['string', 'integer', 'number', 'boolean', 'array'])
                ->description('Required for create/update *_property. Shape follows age_property_type from schema search: STRING/DATE/MONTH_DAY/TIMESTAMPTZ → string; INTEGER → integer; FLOAT → number; BOOLEAN → boolean; ENUM → non-empty string array of option `value`s from enum_options (respect min_selections/max_selections; prefer active options). Empty array is invalid — use delete_*_property to clear.'),
