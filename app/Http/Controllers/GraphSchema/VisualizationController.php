...
class VisualizationController extends Controller
{
    ...
    {
        $vertexTypeList = VertexType::with('properties')->orderBy('id')->get()->map(fn ($vt) => [
            'id' => $vt->id,
            'name' => $vt->name,
            'age_label_name' => $vt->age_label_name,
            'description' => $vt->description,
            'properties' => $vt->properties->map(fn ($p) => [
                'name' => $p->name,
        ...
        $edgeTypeList = EdgeType::with(['properties', 'startVertex', 'endVertex'])->orderBy('id')->get()->map(fn ($et) => [
            'id' => $et->id,
            'name' => $et->name,
            ...
            'properties' => $et->properties->map(fn ($p) => [
                'name' => $p->name,
                'age_property_name' => $p->age_property_name,
                'age_property_type' => $p->age_property_type,
            ]),
