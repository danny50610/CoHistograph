...
| 模式 | 適用場景 | 認證 |
|------|---------|------|
| **Web（HTTP + SSE）** | 遠端 AI 客戶端、生產環境 | Sanctum Personal Access Token；`Authorization: Bearer {token}` |
| **Local（stdio）** | 本機 Cursor / Claude Desktop | 同樣須 Sanctum Token；由 MCP 客戶端設定傳入（如 `.mcp.json` 的 `env`） |

客戶端須先為使用者建立 Sanctum API Token，再連線 MCP Server。初期可先實作 **Local** 模式驗證流程，再擴充 Web 模式。
...
**所有 MCP Tool 皆須登入**，無論 Graph 查詢、Schema 查詢或 Revision 操作。未帶有效 Token 或 Token 無效時回傳 401。

| 模式 | 方案 |
|------|------|
| Web | `Authorization: Bearer {sanctum_token}` |
| Local | 同上；Token 由 MCP 客戶端透過環境變數等方式傳入 |

實作時於 `routes/ai.php` 對 Web 與 Local Server 皆套用 `auth:sanctum` middleware。測試時以 `actingAs($user)` 模擬登入。

### 授權對照表

在**已登入**前提下，各 Tool 另依業務規則檢查：

| MCP 元素 | 權限需求 |
|----------|---------|
| 所有 Tools（共通） | 須登入（Sanctum 認證通過） |
| Graph / Schema 查詢 Tools | 登入即可 |
| `create-revision` | 登入即可 |
...
4. 設定 Sanctum 認證（Web / Local 皆套用 `auth:sanctum`），以 Local 模式用 MCP Inspector 驗證
5. Feature 測試：`tests/Feature/Mcp/GraphToolsTest.php`、`tests/Feature/Mcp/RevisionToolsTest.php`（含未登入 401）
...
| 權限 | 未登入呼叫任一 Tool 回 401；非本人更新修訂回 403 |
| 業務邏輯 | 驗證失敗時 submit 被拒、Schema 搜尋分頁與 `total` 正確 |
