...
│       └── GetRevisionTool.php
├── Resources/
│   ├── GraphSchemaResource.php
│   ├── DomainGuideResource.php
│   └── RevisionResource.php          # Resource Template
└── Prompts/
...
| Name | `CoHistograph` |
| Version | `1.0.0` |
| Instructions | 協作式歷史事件知識圖譜平台。可查詢圖譜 Schema 與頂點資料、協助建立與提交修訂。所有圖資料變更須透過 Revision 工作流，不可直接寫入 AGE。 |
...
flowchart TB
    ...
    subgraph Resources["Resources（唯讀上下文）"]
        R1[graph-schema://overview]
        R2[docs://domain-guide]
        R3[revision://id]
    end
...
**委派**：`RevisionReviewService`。

---

## Resources

Resources 提供 AI 可讀取的唯讀上下文，不需主動呼叫 Tool 即可取得背景知識。

### `graph-schema://overview`（GraphSchemaResource）

| 屬性 | 值 |
|------|-----|
| URI | `graph-schema://overview` |
| MIME | `application/json` |
| Description | 完整的 Graph Schema 定義，含所有 VertexType、EdgeType 及其 Property |

**內容**：序列化 `VertexType::with('properties')` 與 `EdgeType::with(['startVertex', 'endVertex', 'properties'])` 的 JSON。

**更新時機**：每次請求即時查詢（Schema 變更頻率低）。

### `docs://domain-guide`（DomainGuideResource）

| 屬性 | 值 |
|------|-----|
| URI | `docs://domain-guide` |
| MIME | `text/markdown` |
| Description | CoHistograph 領域知識、命名規則與修訂工作流說明 |

**內容**：精簡版領域說明，包含：

- VertexType / EdgeType / Property 概念
- AGE label 命名規則（小寫英數字加底線，最長 64 字元）
- Revision 工作流與支援的 action 類型
- `target_age_id` 與 `target_ref_order` 的語意（參照既有頂點 vs. 同批次新建頂點）

### `revision://{id}`（RevisionResource，Resource Template）

| 屬性 | 值 |
|------|-----|
| URI Template | `revision://{id}` |
| MIME | `application/json` |
| Description | 特定修訂的完整狀態與操作列表 |
...
| `update-revision` / `submit-revision` | 本人（`RevisionPolicy`） |
| `approve-revision` / `reject-revision` | `revision.review` |
| `graph-schema://overview` | 無 |
| `revision://{id}` | `RevisionPolicy::view` |
...
3. 建立 `GraphSchemaResource`、`DomainGuideResource`
4. Local 模式註冊，以 MCP Inspector 驗證
5. Feature 測試：`tests/Feature/Mcp/GraphToolsTest.php`

### Phase 2：Revision 工作流

1. 實作 Revision Tools（create / update / validate / submit / get）
2. 建立 `RevisionResource`（Template）
3. 建立 `DraftHistoricalEventPrompt`
4. Feature 測試：`tests/Feature/Mcp/RevisionToolsTest.php`

### Phase 3：進階功能（可選）

1. Web 模式 + Sanctum 認證
2. 管理員審核 Tools
3. `AnalyzeGraphConnectionsPrompt`
4. Graph Subgraph App
...
| `approve-revision` | `RevisionReviewService::approve` |
