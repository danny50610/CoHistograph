...
2. 建立 `CoHistographServer`，註冊所有 Tools 與 Prompts
3. 實作 Graph / Schema / Revision Tools
4. 實作 `DraftHistoricalEventPrompt`、`AnalyzeGraphConnectionsPrompt`
...
**原則**：Tool 的 `handle()` 應委派至上述 Service / Controller，不在 MCP 層重複業務邏輯。若 Controller 邏輯過於耦合 View，應先抽出 `GraphQueryService`。
