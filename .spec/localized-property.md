...
**locale=null 與 locale 不為 null 並存（髒資料防禦）：**

Form Request 互斥規則保證正常寫入路徑不會混用，但資料庫仍可能因以下途徑產生髒資料：

- migration 後的舊資料補填不完整
- 人工 SQL / Seeder 直接寫入
- 未來 Rule 被繞過的 edge case

因此 Grouper **必須對混用情況採防禦性處理**，而非假設不存在：同一 base name 下若同時存在 `locale=null` 與 `locale!=null` 的 property，兩者分別獨立成組（`locale=null` 自成一組，`locale!=null` 另成一組），不合併，避免顯示錯亂。`LocalizedPropertyGrouperTest` 覆蓋此案例正是基於此理由，而非與 Form Request 互斥規則矛盾。
